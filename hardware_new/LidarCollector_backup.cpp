...
int32_t LMS1Collector::init(char host[], int32_t port)
{
  ...
  return (init_finished_ ? BGS_OK : BGS_ERROR);
}
...
void LMS1Collector::deInit()
{
  ...
  do {
    Bgs::msleep(10);
  } while (!thread_update_data_.joinable());
  thread_update_data_.join();
  // This still not work
  if (laser.isConnected()) {
...
int32_t LMS1Collector::get_scan_msg(Bgs::LaserScanMsg *scan_msg)
{
  scan_msg->header.stamp = BgsTime::get_current_time();
  if (laser.getScanData(&data)) {
    scan_msg->laser_scan_count = data.dist_len1;
    for (int32_t i = 0; i < (int32_t)data.dist_len1; i++) {
      // 这里注释掉了对于反射率的判断
      // 因为有时候会出现反射率都是0的情况，目前还不明原因，需要调查 TODO
      scan_msg->intensities[i] = data.rssi1[i] * 0.001f;
      scan_msg->ranges[i] = data.dist1[i] * 0.001f;
    }
...
int32_t LMS1Collector::get_scan_cfg(Bgs::LaserScanMsg *scan_msg)
{
  ...
  scan_msg->angle_increment =
    (double)outputRange.angleResolution / 10000.0 * BGS_LIDAR_DEG2RAD;
  scan_msg->angle_min =
    (double)outputRange.startAngle / 10000.0 * BGS_LIDAR_DEG2RAD - M_PI_2;
  scan_msg->angle_max =
    (double)outputRange.stopAngle / 10000.0 * BGS_LIDAR_DEG2RAD - M_PI_2;
...
void LMS1Collector::update_data()
{
  ...
  while (!exit_thread_) {
    ...
    if (get_scan_msg(laser_scan_.get()) == BGS_OK) {
      int32_t ret = callback_();
...
int32_t LMS1Collector::get_point_cloud2_cfg(Bgs::PointCloud2Ptr &cloud)
{
  PRINT_ERROR(
...
int32_t LMS1Collector::get_point_cloud2(Bgs::PointCloud2Ptr &cloud)
{
  PRINT_ERROR(
    "It is a 2d laser, and connot provide you with a PointCloudMsg! ");
  return BGS_ERROR;
...
int32_t RoboSenseCollector::get_scan_msg(Bgs::LaserScanMsg *scan_msg)
{
  PRINT_WARNING("it is a 3D lidar, cannot get a scan msg!");
  return BGS_ERROR;
}
int32_t RoboSenseCollector::get_scan_cfg(Bgs::LaserScanMsg *scan_msg)
{
  PRINT_WARNING("it is a 3D lidar, cannot get a scan msg!");
  return BGS_ERROR;
}

int32_t RoboSenseCollector::get_point_cloud2_cfg(Bgs::PointCloud2Ptr &cloud)
{
  return BGS_OK;
}

int32_t RoboSenseCollector::get_point_cloud2(Bgs::PointCloud2Ptr &cloud)
{
  if (lidar_ && convertor_) {
    RslidarScanPtr scan(new RslidarScan);
    if (lidar_->poll(scan))
      convertor_->processScan(scan, cloud);
    else
      return BGS_ERROR;
  } else
    return BGS_ERROR;
...
void RoboSenseCollector::update_data()
{
  int32_t error_count = 0;
  ...
  while (!exit_thread_) {
    ...
    if (get_point_cloud2(point_cloud2_) == BGS_OK) {
      int32_t ret = callback_();
      if (ret > 0) // 并没有报错，但是忽略这一次的操作
      {
        msleep(10);
