...
int32_t CopleyMotor::initialisation(const char *interfacePath)
{
    serial_port_ =
    boost::make_shared<SerialPort>(interfacePath, 9600, 8, 'N', 1);
    if (serial_port_ && serial_port_->is_running())
    PRINT_INFO("Motor initialisation succeeded!");
    else {
        PRINT_ERROR("Motor init failed.");
...
int32_t CopleyMotor::setMode(int mode)
{
    int32_t sizeOfChar = 0;
    char command[64] = { '\0' };
    if (!serial_port_->is_running()) {
    ...
    for (int i = 0; i < kMotorLocCount; ++i) {
        sizeOfChar = sprintf(command, "%d s r0x24 %d\r", i ,mode);
        // printf("set mode complete \n");
        if (serial_port_->write(command, sizeOfChar) == BGS_ERROR) {
            PRINT_WARNING("fail to send command to motor");
            return BGS_ERROR;
        }
...
int32_t CopleyMotor::setSpeed(int16_t leftSpeed, int16_t rightSpeed)
{
    //return 0;
    ...
    int leftSpeednew = leftSpeed * 2000;
    int rightSpeednew = -rightSpeed * 2000;
    // printf("l: %d r: %d \n",leftSpeednew,-rightSpeednew);
    // due to the internal controller send a inverse value to the motor
    // there we need to send a inverse value to internal controller

    const int32_t n_speed[kMotorLocCount] = { rightSpeednew, leftSpeednew };
    for (int i = 0; i < kMotorLocCount; ++i) {
        int32_t sizeOfChar = sprintf(command, "%d s r0x2f %d\r", i, n_speed[i]);
        if (serial_port_->write(command, sizeOfChar) == BGS_ERROR) {
            PRINT_WARNING("fail to send command to motor");
            return BGS_ERROR;
        }
        usleep(20000);
        memset(buffer, 0, sizeof(buffer));
        if (serial_port_->read(buffer, 64) <= 0) {
            PRINT_ERROR("Set speed failed");
            return BGS_ERROR;
        }        
    }
    return BGS_OK;
...
int CopleyMotor::getSpeed(int16_t &leftSpeedIn, int16_t &rightSpeedIn)
{
    // return 0;
    char buffer[64],command[64];
    if (!serial_port_->is_running()) {
    ...
    int n_speed[kMotorLocCount] = { 0, 0 };
    for (int i = 0; i < kMotorLocCount; ++i) {
        int32_t sizeOfChar = sprintf(command, "%d g r0x18\r", i);
        if (serial_port_->write(command, sizeOfChar) == BGS_ERROR) {
            PRINT_WARNING("fail to send command to motor");
            return BGS_ERROR;
        }
        ...
        // for(int i=0;i<ret;i++)printf("%02hhx ",buffer[i]);printf("\n");
        if(sscanf(buffer, "v %d\r", &n_speed[i]) <= 0) {
            char str[10];
            if(sscanf(buffer, "%s\rv %d\r", str, &n_speed[i])<=0){
                PRINT_ERROR("sscanf error"); 
                return BGS_ERROR;           
            }
        }
    }
    rightSpeedIn = -(int)round(n_speed[0]/2000.0);
    leftSpeedIn  = (int)round(n_speed[1]/2000.0);

    return BGS_OK;
}

void CopleyMotor::deInit()
{
    setSpeed(0, 0);
    setMode(0);
    PRINT_INFO("motor stoped");
