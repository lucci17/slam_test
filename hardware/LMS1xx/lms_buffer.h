...
public:
  ...
  {
    int ret =
      read(fd, buffer_ + total_length_, sizeof(buffer_) - total_length_);

    if (ret > 0) {
      total_length_ += ret;
      // PRINT_DEBUG_FMT("Read %d bytes from fd, total length is %d.", ret,
      // total_length_);
    } else {

      // PRINT_WARNING("Buffer read() returned error.");
    }
  }
  ...
  char *getNextBuffer()
  {
    if (total_length_ == 0) {
      ...
      // PRINT_DEBUG("Empty buffer, nothing to return.");
      return NULL;
    }

    // The objective is to have a message starting at the start of the buffer,
    // so if that's not
    // the case, then we look for a start-of-message character and shift the
    // buffer back, discarding
    ...
    if (start_of_message == NULL) {
      ...
      PRINT_WARNING_FMT("No STX found, dropping %d bytes from buffer.",
                        total_length_);
      total_length_ = 0;
    } else if (buffer_ != start_of_message) {
// Start of message found, ahead of the start of buffer. Therefore shift the
// buffer back.
#ifndef __arm__
      PRINT_WARNING_FMT(
        "Shifting buffer, dropping %ld bytes, %ld bytes remain.",
        (start_of_message - buffer_),
        total_length_ - (start_of_message - buffer_));
#else
      PRINT_WARNING_FMT("Shifting buffer, dropping %d bytes, %d bytes remain.",
                        (start_of_message - buffer_),
                        total_length_ - (start_of_message - buffer_));
#endif
    ...
    if (end_of_first_message_ == NULL) {
      ...
      // PRINT_DEBUG("No ETX found, nothing to return.");
      return NULL;
    }
