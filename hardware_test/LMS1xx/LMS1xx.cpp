...
{
  if (!connected_) {
    PRINT_INFO("Creating non-blocking socket.");
    ...
    if (socket_fd_) {
      ...
      PRINT_INFO("Connecting socket to laser.");
      int ret = ::connect(socket_fd_, (struct sockaddr *)&stSockAddr,
                          sizeof(stSockAddr));

      if (ret == 0) {
        connected_ = true;
        PRINT_INFO("Connected succeeded.");
      }
    }
  }
}
...
void LMS1xx::startMeas()
{
  ...
  (void)write(socket_fd_, buf, strlen(buf));

  int len = read(socket_fd_, buf, 100);
  if (buf[0] != 0x02)
    PRINT_WARNING("invalid packet recieved");
  buf[len] = 0;
  //   PRINT_DEBUG_FMT("RX: %s", buf);
}
...
void LMS1xx::scanContinous(int start)
{
  ...
  if (buf[0] != 0x02)
    PRINT_ERROR("invalid packet recieved");
...
bool LMS1xx::getScanData(scanData *scan_data)
{
  ...
  while (1) {
    // Would be great to depend on linux's behaviour of updating the timeval,
    // but unfortunately
    ...
    // PRINT_DEBUG_FMT("returned %d from select()", retval);
    if (retval < 0) {
      PRINT_ERROR("Poll Error!");
      return false;
    } else if (retval == 0) {
      PRINT_WARNING("Poll Timeout!");
      return false;
    } else {
...
void LMS1xx::parseScanData(char *buffer, scanData *data)
{
  ...
  // PRINT_DEBUG_FMT("NumberChannels16Bit : %d", NumberChannels16Bit);
  ...
  for (int i = 0; i < NumberChannels16Bit; i++) {
    ...
    // PRINT_DEBUG_FMT("NumberData : %d", NumberData);
  ...
  // PRINT_DEBUG_FMT("NumberChannels8Bit : %d\n", NumberChannels8Bit);
