/*!
 * @file bgs_time.h
 * @brief header for class BgsTime, BgsDuration
 *
 * @author edward.liu
 * @date 2018-10-08
 *
 ...
 *
 * it is like the class BgsTime but it's simpler
...
public:
  ...
  /*!
   * \brief get current system time into a BgsTime instance
   */
  static BgsTime get_current_time()
  {
    struct timespec ts;
    clock_gettime(CLOCK_REALTIME, &ts);

    return BgsTime(ts.tv_sec, ts.tv_nsec);
  }

  /*!
   * \example bgs_time.h
   * this is a example for static function in BgsTime: for_sec(double sec)
   * * 2.03 -> BgsTime: secs = 2; nsecs = 0.03 * 1000000000;
   * * -2.03 -> BgsTime: secs = 0; nsecs = 0;
   *
   * this is a example for static function in BgsTime: fromNSec(double sec)
   * * 2300000000 -> BgsTime: secs = 2; nsecs = 300000000;
   */
  /*!
   * \brief get a BgsTime instance from time in second
   */
  static BgsTime from_sec(double sec)
  {
    BgsTime time;
  ...
  /*!
   * \brief get a BgsTime instance from time in ns
   */
  static BgsTime fromNSec(uint64_t t)
  {
    return BgsTime((uint32_t)(t / 1000000000), (uint32_t)(t % 1000000000));
  }

  /*!
   * \brief get a BgsTime instance with the max value
   */
  static BgsTime TIME_MAX()
  {
    return BgsTime((uint32_t)(0xFFFFFFFF), (uint32_t)999999999);
  }

  BgsTime &operator=(const BgsTime &b)
  {
    this->nsecs = b.nsecs;
    this->secs = b.secs;
    return *this;
  ...
  bool operator!=(const BgsTime &b) const { return !((*this) == b); }

  // 判断时间相同有一个阈值，直接判断相等通常来说都不相等
  // 这里暂时定阈值为 1us
  // TODO 后面的大小判断都应该有这个阈值，这个阈值也应该可以设定
  bool operator==(const BgsTime &b) const
  {
   ...
   */
  BgsTime operator+(const BgsTime &b) const
  {
   ...
   * \warning there is risk of overflow
   */
  BgsTime operator+(const BgsDuration &b) const
  {
    int64_t tmp_nsecs = this->toNSec() + b.toNSec();
    return fromNSec(tmp_nsecs);
  }
   ...
   */
  bool operator<(const BgsTime &b) const { return (toNSec() < b.toNSec()); }
  bool operator<=(const BgsTime &b) const { return (toNSec() <= b.toNSec()); }
  bool operator>(const BgsTime &b) const { return (toNSec() > b.toNSec()); }
  bool operator>=(const BgsTime &b) const { return (toNSec() >= b.toNSec()); }

  friend std::ostream &operator<<(std::ostream &os, BgsTime t1)
  {
  ...
  /*!
   * \brief transfrom the BgsTime to a double value (seconds)
   */
  ...
  /*!
   * \brief transfrom the BgsTime to a int value (ns)
   */
  inline int64_t toNSec() const
  {
    return ((int64_t)secs * 1000000000 + (int64_t)nsecs);
