...
#ifdef MATRIX_USE_SYCL
using namespace cl::sycl::access;
using cl::sycl::nd_item;
using cl::sycl::item;
using cl::sycl::range;
using cl::sycl::nd_range;
 ...
 * */
bool local_mxm( cl::sycl::queue& q, const Matrix& m_a, const Matrix& m_b, Matrix& m_c ) 
{
	int32_t m = m_a.m;
	int32_t k = m_a.n;
	int32_t n = m_b.n;
...
// 	int32_t maxBlockSize = device.get_info<cl::sycl::info::device::max_work_group_size>();
	...
	// start sycl loop
	{
		...
		* "zero-copy" memory optimisations can be used by the driver. */
		...
		cl::sycl::buffer<FLOAT> bA(m_a.val[0], cl::sycl::range<1>(m * k), props);
		cl::sycl::buffer<FLOAT> bB(m_b.val[0], cl::sycl::range<1>(k * n), props);
		cl::sycl::buffer<FLOAT> bC(m_c.val[0], cl::sycl::range<1>(m * n), props);

#if 0
		q.submit([&](cl::sycl::handler& cgh) 
		{
			auto pA = bA.get_access<mode::read>(cgh);
			auto pB = bB.get_access<mode::read>(cgh);
			auto pC = bC.get_access<mode::discard_write>(cgh);
			
			cgh.parallel_for<class mxm_kernel>( 
				cl::sycl::nd_range<2>{cl::sycl::range<2>(m, n), 
					cl::sycl::range<2>(1, 1)}
				, [=](nd_item<2> it)
			{			
				int32_t row = it.get_global(0);
				int32_t col = it.get_global(1);
				
				FLOAT sum = 0.f;
				for( int32_t i = 0; i < k; ++i )
				{
					sum += pA[row*k+i]*pB[i*n+col];
		...
		q.submit([&](cl::sycl::handler& cgh) 
		{
			...
			auto myRange = nd_range<1>(range<1>(m*n), range<1>(1));
			
			cgh.parallel_for<class mxm_kernel>( 
				myRange, [=](nd_item<1> it)
			{			
				int32_t index = it.get_global(0);
				int32_t row = index / n;
				int32_t col = index % n;
...
cl::sycl::gpu_selector Matrix::selector_;
cl::sycl::queue Matrix::sycl_queue_(Matrix::selector_);
...
// void Matrix::setValWithIndex(FLOAT val_, int32_t i, int32_t j)
